...
def main():
    ...
    plan=dict(input_sha1=sh,file=ent['path'],extent_lba=ent['extent'],file_size=ent['size'],candidate_offset=off,candidate_length=length,candidate_raw_bin_offset=raw_offset(ent['extent'],off),candidate_lbas=sorted(patch_secs),bdp_ancestor_count=len(anc),checksum_lbas=sorted(checksum_secs),all_touched_lbas=touched,replacement_simulated=bool(replacement),checksum_changes=[dict(container_start=s,container_end=e,old=old,new=new) for s,e,old,new in changes],simulated_logical_file_sha1=patched_sha)
