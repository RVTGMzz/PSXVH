...
B33 = TR / "BATCH33_FINAL_EXACT_SET_0.6.43.0.csv"
...
OUT = TR / "BATCH35_RESIDUAL_PRIORITY_0.6.45.0.csv"
REPORT = ROOT / "checkpoints" / VERSION / "BATCH35_RESIDUAL_PRIORITY_REPORT.txt"
...
def main():
    ...
    lines = [
        f"GAIA MASTER {VERSION} BATCH 35 POST-BATCH34 RESIDUAL PRIORITY",
        "=" * 84,
        f"Translation Master rows      : {len(master)}",
        f"Protected B33 exact keys     : {len(protected_b33)}",
    ...
    lines += ["", "TOP 60 PRIORITY CLUSTERS:"]
    for i, r in enumerate(out_rows[:60], 1):
        lines.append(
    ...
    lines += [
        ...
        "- protection uses B33 final exact set plus proven historical locks",
        "- next batch should curate the highest-value remaining clusters",
        "",
