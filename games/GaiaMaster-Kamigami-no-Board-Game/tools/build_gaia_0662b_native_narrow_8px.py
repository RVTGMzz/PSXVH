...
import os,sys,shutil,struct,unicodedata,re
...
BUILD_LABEL="0.6.6.2b NATIVE NARROW STRICT GATE"
...
# Manual corpus filter before this proof:
#   EXCLUDED with confirmed real-text use: idx07 '(' , idx08 ')' , idx10 '+'
#   EXCLUDED from earlier design: idx00 '!' , idx04 '%' , idx11 ','/'-' , idx14 '.'/'/'
# Remaining eight diagnostic owners:
CANDIDATES=[(1,0x22,'"'),(2,0x23,"#"),(3,0x24,"$"),(5,0x26,"&"),(6,0x27,"'"),(9,0x2A,"*"),(12,0x5A,"Z"),(13,0x58,"X")]
...
def textlike(s):
    """Strict diagnostic text classifier.

    Previous 0.6.6.2 used `ascii_alpha >= 2`, which accepted binary garbage
    such as 62R#eUReVR\"T\"WTV and caused a false safety block.

    Here:
    - any decoded string containing Japanese text is considered real;
    - ASCII-only strings are considered real only when they look like a
      compact UI/debug identifier rather than random mixed-case binary.
    """
    if len(s)<2 or not all(c.isprintable() for c in s):
        return False
    ...
    if not all(ord(c)<128 for c in s):
        return False
    if not re.fullmatch(r'[A-Z0-9 _./:+!?%()\-]{2,40}', s):
        return False
    alpha=sum('A'<=c<='Z' for c in s)
    return alpha>=2
...
def main():
    ...
    gate=os.path.join(here,"GaiaMaster_0.6.6.2b_NATIVE_NARROW_GATE_REPORT.txt")
    report=["GAIA MASTER 0.6.6.2b NATIVE NARROW 8PX STRICT-GATE PROOF","="*78,"DATA ONLY / 6x12 / NO HOOK / NO CAVE / NO CURSOR PATCH / STRICT TEXT GATE","expected: "+TEST_TEXT,""]
    def save(path=gate):
    ...
    report += [
        ...
        "GATE NOTE",
        "-"*78,
        "0.6.6.2 old gate false-positive examples were binary-looking CP932 runs.",
        "Confirmed real-text owners removed before this build: '(' , ')' , '+'.",
        "This remains a Character Select diagnostic proof only, not production allocation.",
    ]
    ...
    stem=os.path.splitext(src)[0]; ob=stem+" [VI 0.6.6.2b NATIVE NARROW 8PX].bin"; oc=stem+" [VI 0.6.6.2b NATIVE NARROW 8PX].cue"; ot=stem+" [VI 0.6.6.2b NATIVE NARROW 8PX].txt"
    ...
    save(ot); save(); print("[OK] 0.6.6.2b NATIVE NARROW 8PX BUILD SUCCESS"); print("Output:",oc); print("Report:",ot); return 0
