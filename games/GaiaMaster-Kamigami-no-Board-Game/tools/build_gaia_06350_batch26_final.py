...
from batch26_stage_06350 import build_plan, staged_sources
...
def selftest():
    required = [INNER, BASE_READABLE, B20, B24, *PARTS]
    missing = [str(p) for p in required if not p.is_file()]
    if missing:
        die("Missing Batch26 builder dependencies:\n  " + "\n  ".join(missing))
    plan = build_plan()
    if len(final_expected_map()) != EXPECTED_FINAL:
        die("Final 222-key manifest gate failed")
    base = load_base()
    ...
    with staged_sources(plan):
        pass
    print("=" * 84)
    print("GAIA MASTER 0.6.35.0 BATCH26 BUILDER SELFTEST PASS")
    print("=" * 84)
    ...
    print(f"Final output verify   : {EXPECTED_FINAL} exact keys required")
...
def main():
    ...
    print("=" * 84)
    print("GAIA MASTER 0.6.35.0 - BATCH26 FINAL EXACT PRODUCTION BUILD")
    print("=" * 84)
    ...
    report = out_dir / f"GaiaMaster_{VERSION}_BATCH26_FINAL_REPORT.txt"
    lines = [
        f"GAIA MASTER {VERSION} - BATCH26 FINAL EXACT PRODUCTION BUILD",
        "=" * 84,
        f"Input CLEAN SHA1: {got_sha}",
        f"Batch25 new exact targets staged: {len(plan['targets'])}",
    ...
    print()
    print(f"[OK] {VERSION} Batch26 production build completed")
    print(f"[OK] Final exact byte verification: {verified}/{EXPECTED_FINAL}")
    print(f"[OK] Sources restored after staging")
