...
def selftest():
    ...
    print('B52R28 CD DMA PROVENANCE ANALYZER SELFTEST PASS')
...
def main():
    ...
    if not a.trace:ap.error('B52R28 CD_DMA_TRACE.tsv required')
    ...
    lines=['GAIA MASTER B52R28 CD DMA PROVENANCE ANALYSIS','='*78,f'events          : {len(rows)}',f'overlap events  : {len(hits)}',f'BIN SHA1        : {sh}',f'ISO files       : {len(files)}','']
    ...
    out=a.trace.with_name('GaiaMaster_B52R28_CD_DMA_PROVENANCE_ANALYSIS.txt'); out.write_text('\n'.join(lines)+'\n',encoding='utf-8'); print('\n'.join(lines)); print('Report:',out)
    return 0
