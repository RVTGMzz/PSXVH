...
def main():
 ...
 out=p.parent; hp=out/"GaiaMaster_B52R23_GPU_MMIO_HITS.csv"; cp=out/"GaiaMaster_B52R23_GPU_COMMAND_BUILDERS.csv"; bp=out/"GaiaMaster_B52R23_GPU_BREAKPOINTS.txt"; lp=out/"GaiaMaster_B52R23_PCSX_BREAKPOINTS.lua"; rp=out/"GaiaMaster_B52R23_GPU_UPLOAD_LOCATOR_REPORT.txt"
 ...
 bp.write_text("\n".join(bl)+"\n",encoding="utf-8")
 targets=scored[:10]
 lua=["-- Auto-generated by Gaia Master B52R23 GPU upload locator","local gaia_bps = {}","gaia_trace_armed = false","function gaia_arm() gaia_trace_armed=true; print('GAIA B52R23 trace ARMED') end","function gaia_disarm() gaia_trace_armed=false; print('GAIA B52R23 trace disarmed') end","local targets = {"]
 for s,fs,names,hs in targets:lua.append(f"  {{addr=0x{fs:08X}, score={s}, label='{','.join(sorted(names))}'}},")
 lua += ["}","for i,t in ipairs(targets) do","  gaia_bps[i] = PCSX.addBreakpoint(t.addr, 'Exec', 4, 'Gaia B52R23 '..t.label, function()","    if not gaia_trace_armed then return end","    local r = PCSX.getRegisters()","    print(string.format('GAIA_B52R23 HIT label=%s score=%d pc=%08X ra=%08X sp=%08X', t.label, t.score, r.pc, r.GPR.n.ra, r.GPR.n.sp))","    PCSX.pauseEmulator()","    return false","  end)","end","print('GAIA B52R23 breakpoints loaded. Run gaia_arm() just before the target menu.')"]
 lp.write_text("\n".join(lua)+"\n",encoding="utf-8")
