...
11. Nếu cần CD provenance sâu hơn, build B52R28 từ B52R24 TRACE, arm Lua trước transition, rồi analyze CD_DMA_TRACE.tsv với exact B52R14R1 để map Setloc/LBA → ISO owner candidate.
12. Nếu B52R25 writer PC ở ngoài main SLPS, chạy B52R29 với WRITER_TRACE.tsv + WRITER_RAM.bin + exact BIN để fingerprint overlay code owner.
13. Khi đã có real ownership candidate, chạy B52R30 để verify ISO identity + BDP ancestry + checksum/touched-sector plan. Nếu có replacement same-length, chỉ simulate trong B52R30 trước.
14. B52R31 trước hết chạy DRY-RUN. Chỉ dùng BUILD launcher sau khi target-frame ownership đã được chứng minh và B52R30 plan khớp tuyệt đối.
15. Runtime PASS toàn game vẫn NO cho tới khi user test gameplay.
